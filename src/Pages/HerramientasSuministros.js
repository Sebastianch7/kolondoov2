...
import { getSuministros } from '../services/ApiServices';
...
export default function HerramientasSuministros() {
    ...
    useEffect(() => {
        ...
        const fetchBlogList = async () => {
            setIsLoading(true);
            try {
                const response = await getSuministros();
                ...
                <Row>
                        ...
                        <Row>
                            <Col xs={12}>
                                ...
                                <p className='my-md-3 px-2 p-md-0'>
                                    <b>Los suministros básicos de cada hogar son la <a href='/es-es/blog/energia/tarifa-indexada-y-pvpc-diferencias' target='_blank'>luz</a>, el <a href='/es-es/blog/energia/ahorrar-en-tarifa-de-gas-natural' target='_blank'>gas</a> y el agua potable.</b> En otras palabras, son los servicios más esenciales en una vivienda que cubren las necesidades más importantes de nuestro día a día.
                                </p>
                                <p className='my-md-3 px-2 p-md-0'><b>Además, a estos servicios podríamos sumarle en los últimos años Internet, ya que cada vez se vuelve más necesario contratar una <a href='/es-es/internet-telefonia/comparador-fibra' target='_blank'>tarifa de fibra óptica</a></b> (o ADSL si aún no tienes cobertura de fibra en tu edificio), y es que está normalizándose teletrabajar a tiempo parcial o completo.</p>
                            ...
                            <Col xs={12}>
                                ...
                                <p className='my-md-3 px-2 p-md-0'>Desde Kolondoo te recomendamos que <b>compruebes el <a href="/es-es/herramientas/precio-de-la-luz-hoy" target='_blank'>precio de la luz por horas</a></b>, para ser consciente de cuándo está más barata y más cara la electricidad y puedas ahorrar en tu factura.</p>
                                <p className='my-md-3 px-2 p-md-0'>Continuando por el gas, de acuerdo con el <a href='https://aprendecomoahorrarenergia.es/campus/index.php/' target='_blank' rel='nofollow noopener noreferrer'>Instituto para la Diversificación y Ahorro de la Energía (IDEA)</a>, en las viviendas de nuestro país que no disponen de <a href='/es-es/blog/hogar/ahorrar-calefaccion-gas-natural-trucos'><b>servicio de calefacción,</b></a> <b>el consumo medio mensual de gas natural es de 425,75 kWh, mientras que en los hogares que sí cuentan con calefacción de gas llega a 660 kWh.</b></p>
                                    ...
                                    <Card.Body>
                                        <ul className='icon-list'>
                                            ...
                                            {fetchBlog?.map((item, index) => {
                                                return < a target='_blank' key={index} href={`/es-es/herramientas/suministros/${item.url_amigable}`}><li className='m-3 font-semibold'>{`${item.titulo}`}</li></a>
                                            })
                                            }
                                        </ul>
                                    </Card.Body>
                    ...
                    <Col xs={12} md={4}>
                        ...
                        />
                        {
                            fetchBlog?.length > 0 ?
                                fetchBlog.map((item, index) => {
                                    return <TarjetaBlogMin categorias={'Suministros'} key={index} data={item} />
                                })
                                :
                                <Load></Load>
                        }
